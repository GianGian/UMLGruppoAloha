...
void OrdineAcquisto::addProdotto(int prezzo, int quantita, Prodotto *p){
	if(p == NULL){
		cout<<"impossibile aggiungere il prodotto all'ordine. E' inesistente?";
	}else{
...
ostream& operator<< (ostream& os, OrdineAcquisto &oa){
	os<<"Ordine acquisto"<<" n."<<oa.get_n_ordine()<<" del " <<oa.get_data()<<" fatto da "<<oa.get_matricola()<<" con prodotti:"<<endl;
